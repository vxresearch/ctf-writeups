...
# start loop
k = [[0 for _ in range(8)] for _ in range(5)]
k[4] = [130,167,150,65,-1,-1,194,40]
#for x in range(256):
#    print "x: ", x
#    for y in range(256):
for i in range(256):
...
#                k[4][2] = y
        ...
        for r in range(3,-1,-1):
            for b in range(4,8):
                k[r][b] = k[r+1][b]^k[r+1][b-4]
            for b in range(0,4):
                k[r][b] = sbox[k[r][((b+1)%4)+4]]^k[r+1][b]
            k[r][0] = k[r][0]^rcon[r+1]
        # test key integrity
        secret = "".join([chr(q) for q in k[0]])
        c = zer0SPN(k[0])
        assert c.get_roundkey(4) == bytearray(k[4])
        flag = True
        #test
        for ii in range(65536):
            pi = bytearray(plaintext[ii])
            ci = c.encrypt(pi)
            if (pack('8B', *ci) != ciphertext[ii]):
                flag = False
                break
            """
            else:
